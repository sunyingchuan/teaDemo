...
@Order(11)  //设置切面的优先级：如果有多个切面，可通过设置优先级控制切面的执行顺序（数值越小，优先级越高）
@Component //该标签把LoggerAspect类放到IOC容器中
public class AddressAdditionAspect {
     ...
     */
    @Pointcut("execution(public * com.ucar.team.seven.tea.address.service.impl.AdressServiceImpl.insert*(..))")
    public void addAddressJoinPointExpression() {
    }

    @Pointcut("execution(public * com.ucar.team.seven.tea.address.service.impl.AdressServiceImpl.update*(..))")
    public void updateAddressJoinPointExpression() {
    }


    @Pointcut("addAddressJoinPointExpression() || updateAddressJoinPointExpression()")
